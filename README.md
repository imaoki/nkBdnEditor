...
キャラクターの体や手足等にコライダーをリンクさせたい場合に使用します。
...
ベイクしたコントロールノードからアニメーションを削除し、ダイナミクスユニットと再接続します。
現在のところBaseAnimationレイヤにのみ属している前提で正しく機能します。
...
一つのboneDynamicsNodeには必ず終端のジョイントが必要です。
